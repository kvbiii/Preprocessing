...
%macro Perturbation(algorithm, dataset, target_name, metric=AUC, n_repeats=5);
	ods exclude all;	
	...
	quit;
	ods exclude all;
	proc logistic data=work.train outmodel=work.fitted_model;    
	...
	%do i=1 %to %sysevalf(&liczba_zmiennych);
		proc univariate data=work.valid noprint;
			var %scan(&nazwy_zmiennych, &i, ' ');
			output out=work.temp_table pctlpre=P_ pctlpts=50;
		run;
		proc sql noprint;
			select * into :median trimmed from work.temp_table;
		quit;
		%let suma_inner=0;
		%do j=1 %to &n_repeats;
			data work.valid_copy;
				set work.valid;
				%scan(&nazwy_zmiennych, &i, ' ') = %scan(&nazwy_zmiennych, &i, ' ') + rand("Uniform", %sysevalf(-1*&median), &median);
			run;
			ods exclude all;
			proc logistic inmodel=work.fitted_model;
		    	score data=work.valid_copy out=work.predictions fitstat;
			...
			quit;
			%let suma_inner=%sysevalf(&suma_inner+&inner_score);
		%end;
		%let feature_importance&i=%sysevalf(&original_score-&suma_inner/&n_repeats);
	%end;
	*Feature importance można interpretować jako wielkość utraty mocy predykcyjnej modelu mierzona za pomocą metryki &metric na skutek zmiany rozkładku zmiennej &i; 
	...
	run;
%mend;
%Perturbation(algorithm=logistic, dataset=dataset, target_name=y);
