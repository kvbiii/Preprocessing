...
class Other_Plots():
    def __init__(self) -> None:
        pass
    def cross_validation_split(self, X, y, n_splits, cv):
        fig = go.Figure()
        if(len(X) > 100):
            symbol = "line-ns"
        ...
        fig.update_layout(template="simple_white",  xaxis_title="Indices", width=1000, height=1000, font=dict(family="Times New Roman",size=16,color="Black"),  yaxis=dict(ticks="outside", tickvals=[i for i in range(1, n_splits+1)], ticktext=[f"Fold {i}" for i in range(1, n_splits+1)]))
        fig.show("png")